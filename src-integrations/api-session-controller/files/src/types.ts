...
export interface SessionCreateRequest {
  ...
  /**
   * Session-local model selection applied to this Agent's next request.
   * Distinct from {@link SessionSelectModelRequest}: it is never saved as the
   * default, so a caller-supplied route cannot leak into unrelated sessions.
   */
...
export interface SessionPromptRequest {
  ...
  /**
   * Session-local model selection applied to this request's turn (and kept for
   * the session's later requests). Never saved as the default.
   */
  readonly model?: ModelSelection
}
