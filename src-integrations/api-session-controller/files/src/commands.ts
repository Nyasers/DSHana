...
export class SessionCommandController {
  ...
  async create(request: SessionCreateRequest): Promise<SessionCreateValue> {
    ...
    await this.applyRequestSelection(adopted, request.model)
    const agentPreset = this.agents.presetForSession(adopted.session)
    return { sessionId, ...(agentPreset === undefined ? {} : { agentPreset }) }
  }

  /**
   * Validate one request-supplied selection and install it for the next request.
   * A request-supplied route is Session-local: unlike {@link selectModel} it is
   * never saved as the Agent default.
   * @param agent - live Agent that owns the selection.
   * @param model - selection supplied with the request; absent when it carries none.
   * @returns nothing; throws `session/model-unavailable` for an unserved route.
   */
  private async applyRequestSelection(agent: Agent, model: ModelSelection | undefined): Promise<void> {
    ...
    try {
      ...
      this.agents.selectForNextRequest(agent, {
  ...
  async prompt(request: SessionPromptRequest): Promise<SessionPromptValue> {
    ...
    await this.applyRequestSelection(agent, request.model)
