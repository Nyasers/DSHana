...
/**
 * Refuse a working directory that no process can start in.
 *
 * Every spawn form starts its child inside the caller's directory, and that directory
 * can be gone by the time it starts (a scratch tree, a deleted checkout, a moved
 * project). Each platform then fails in its own opaque way: ConPTY reports the
 * CreateProcess ERROR_DIRECTORY as a bare child exit code, and a POSIX child that
 * cannot chdir dies before running anything. Checking here names the directory instead
 * of leaving that outcome to be read as a missing program.
 * @param cwd - working directory about to be handed to a platform launcher.
 * @throws a Node-shaped `ENOENT`/`ENOTDIR` error carrying the directory as `path`.
 */
function requireUsableWorkingDirectory(cwd: string): void {
  let stats: Stats
  try {
    stats = statSync(cwd)
  } catch (cause) {
    throw workingDirectoryError(cwd, 'ENOENT', 'no such file or directory', cause)
  }
  if (!stats.isDirectory()) throw workingDirectoryError(cwd, 'ENOTDIR', 'not a directory')
}

/** The chdir-shaped failure for an unusable working directory: same code, path, and syscall a failed chdir reports. */
function workingDirectoryError(cwd: string, code: 'ENOENT' | 'ENOTDIR', detail: string, cause?: unknown): NodeJS.ErrnoException {
  const error = new Error(`${code}: ${detail}, chdir '${cwd}'`) as NodeJS.ErrnoException
  error.code = code
  error.path = cwd
  error.syscall = 'chdir'
...
export class LocalSubprocessRuntime extends SubprocessRuntime {
  ...
  spawn(spec: SubprocessSpawnSpec): SubprocessHandle {
    ...
    requireUsableWorkingDirectory(spec.cwd)
