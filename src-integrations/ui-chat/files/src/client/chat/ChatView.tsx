...
export function ChatView({
  ...
  // DSHana delta：本次翻页是「滚到顶自动续页」发起的（区别于按钮发起）。
  const scrollTriggeredOlderRef = useRef(false)
  ...
  onScrollRef.current = () => {
    ...
    observedTopRef.current = el.scrollTop
    scheduleActiveTurn()
    // DSHana delta：滚到顶就续页。上游只给了「加载更早」按钮，长会话回翻要一直去够
    // 那个按钮；这里在读者自己滞到顶时走与按钮**同一个入口**（loadOlderAnchored
    // 含锚定补偿，续下的页不会把阅读位顶跑）。三个安全前提：
    //   · movedByReader：只有读者输入算数。续页后的锚定补偿是程序化写入（自己记进
    //     observedTopRef），不算读者移动，不会连轴触发；
    //   · 距离 ≤ LOAD_OLDER_TOP_PX：贴顶才算，正常阅读不误触；
    //   · loadOlder() 自身对「未打开 / 没有更多 / 正在加载」短路，重复触发无害。
    if (movedByReader && el.scrollTop <= LOAD_OLDER_TOP_PX) {
      scrollTriggeredOlderRef.current = true
      loadOlderAnchored()
    }
  }
  ...
  // DSHana delta：滚动触发的续页在提交时让补偿当帧落地（而不是等 500ms 定时器或 scrollend）。
  // 上游那个 layout effect 在 scrollSamplePendingRef 为真时整段早退（它把这个标志当作
  // 「读者手势还没采完」），而滚到顶续页恰好是「读者还在滚 + 页马上提交」的重合场景：
  // 补偿被推迟 = 续下来的页先把阅读位顶一下再弹回。这里在提交时清掉 Pending 并推进
  // scrollSampleTick，让补偿走既有采样路径立即重跑；只作用于本 delta 发起的请求。
  useEffect(() => {
    if (loadingOlder || !scrollTriggeredOlderRef.current) return
    scrollTriggeredOlderRef.current = false
