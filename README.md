...
1. **拖入 zip 包**：把 release zip（`dshana-v<version>.zip`，从 GitHub Releases 下载）拖进 HanaAgent 的拓展安装界面；**批准安装审阅卡上的权限**（运行、本机运行、联网、资源读取；本机运行档位无文件系统隔离，审阅卡会如实标注）。
...
1. **安装**：Release 里除通用包（`dshana-v<version>.zip`）外，还有各平台包（`dshana-v<version>-<target>.zip`，体积明显更小）。按平台取件与安装的完整流程见 [`dshana-install-skill/SKILL.md`](dshana-install-skill/SKILL.md)：取 `index.v2.json` → 选本机 target → 核 sha256 → 先卸旧版 → 走宿主端点安装 → 验证。那份手册可以直接交给 Agent 执行。
...
| 项 | 默认 | 说明 |
| --- | --- | --- |
| 审批超时 | `30` | 审批挂起超时无人应答即自动 rejected；`0` 禁用 |
| 任务默认超时 | `1800` | 单次任务默认超时（秒） |
| 模型 | - | DSHana 任务使用的默认模型 |
| 推理强度 | - | 所用模型的默认推理强度 |
...
DSH 主题偏好为 `system` 时跟随宿主配色；在 DSH 内显式选 light/dark 时完全用 DSH WebUI 自己的主题。宿主切主题后已打开的页面实时跟随。
