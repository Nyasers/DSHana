...
2. **打开 DSHana 卡**：App 加载后自动拉起受管 runtime，卡上按三态渲染：**未启动 / 启动中 / 就绪（装载 DSH Web UI）**。失败会进入「需要处理」态，卡上给出原因与原始错误；打开本卡会补一次启动请求，之后由自动退避重试兜着，端口占用会自动换端口。
...
App 级设置由 App 自绘设置页承载，在宿主设置区渲染，不依赖 DSH 运行；页面分「常规」与「会话模型」两节：

| 项 | 默认 | 说明 |
| --- | --- | --- |
| 审批超时 | `30` | 单位秒；无人应答即自动拒绝，填 `0` 禁用 |
| 任务超时 | `1800` | 单位秒；单次任务超时，填 `0` 按缺省 `1800` 兜底 |
| 会话模型 | 复用调用方 | 工具建的新会话用哪个模型：复用调用方 = 用发起这次调用的那张角色卡配的；自定义模型 = 固定用下面选的那条 |
| 模型 | （空） | 选「自定义模型」时出现；候选列宿主目录里的 provider 与模型 |
| 推理强度 | 留空 | 选「自定义模型」时出现，该模型支持的档位；留空沿用 DSH 当前设置 |

DSH 数据固定落在 App 内置独立目录，开箱即用；共享或切换已有 DSH 目录暂不提供。

**无需配置 API Key / 模型**：推理经受管 runtime 内宿主 `ctx.models` 发起，provider 凭据不进 DSH 进程。DSH 自己那格默认模型（`DSH_HOME/settings.yaml` 的 `agent-default-model`）由 DSH 自己与对账维护，App 设置页不经手；`dshana` 的 `provider` / `model` / `reasoningEffort` 参数可显式覆盖单次任务。
