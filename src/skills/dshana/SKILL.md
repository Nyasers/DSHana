...
- **会话清单与打开**（App 设置页「会话」）：列出本 App 提交过的 DSH 会话（坐标取自宿主任务记录 `metadata.dsh.sessionId`，见 `GET /dshana/sessions`），每条可在**新窗口**里打开（`POST /dshana/sessions/open` → `ctx.windows.create`，manifest 能力 `app/windows.manage`）。窗口加载 `ui/stream.html` 并按读回的 `sid` 钉住那段会话（窗口 entry 是纯 ui 路径，坐标经窗口 `data` 下发，窗口页自己从页面路径取 surface 凭据）。这是当前唯一不依赖宿主入口的「打开」路径：聊天流底部那个任务 chip 点开去哪由宿主决定，App 控制不了。
