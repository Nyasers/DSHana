...
//
// tests/product-package.test.mjs — 交付树 package.json 的字段契约
// （scripts/release/pack/assert.mts 的 assertProductPackage + packaging/package.json）
//
// 守的是「构建输入不进安装包」：仓库那份带着 scripts/devDependencies/packageManager/imports，
// 装机侧没有消费方，混进去只会让人读出错觉。交付树那份只有 name/version/type 三件。
...
test("构建输入字段混进来就拒包（scripts/devDependencies/packageManager/imports/dependencies）", () => {
  for (const extra of [
