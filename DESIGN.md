...
- **会话卡（`ui/stream.html`）的生命周期**：卡面钉住的 DSH 会话 id 有两处来路——iframe 卡走查询串（`?sid=`），原生窗口走窗口 `data`（见上条）；注入的 DSH UI 钉在那一段上，壳页在顶部挂一行跟踪态（取 `GET /dshana/card-state`：只在 `tracked` / `cancelling` 期间慢轮询 4s，其余状态停手）。**终态之后不断流**：这段会话多半是用户主动打开来翻看的历史会话，断掉或拒绝重开就等于看不了。主卡与 FP 不带 `sid`，不挂跟踪态。
...
- **会话清单与打开**：「会话」区块列本 App 提交过的 DSH 会话（`GET /dshana/sessions`，投影见 `src/lib/session-list.ts`：坐标取宿主任务记录的 `metadata.dsh.sessionId`，按最近活动降序、上限 20），每条一个「在新窗口打开」按钮走 `POST /dshana/sessions/open`。后端用 `ctx.windows.create`（能力 `app/windows.manage`）开一个原生窗口：entry 是纯 ui 路径 `/stream.html`，坐标经窗口 `data` 下发。宿主的 entry 校验拒收查询串、片段、反斜杠与百分号编码，而凭据也不该由后端转交：窗口文档由宿主另发一份路径租约（`/api/apps/<id>/ui/_surface/<lease>/<entry>`），surface 凭据就在那段路径里。窗口页用 `hana.window.getContext()` 读回坐标，票则从自己的页面路径取。SDK 为此选 `@hana/app-sdk/ui`（`@hana/plugin-sdk` 不认 `window.hanaAppWindow` 桥，且它的 `hana.api.fetch` 只读查询串，在窗口页必抛）。**不带 `tid`**：卡页拿它当流票面，而中继闸门判的是「宿主任务还活跃」（终态即失活、拒建流），带上就等于打开历史会话时看不到东西；窗口走无票的流（与主卡 / FP 同侧，不闸）。这是**不依赖宿主入口**的打开路径：聊天流底部的任务 chip 点开去哪由宿主决定（宿主侧诉求单独附，不进仓）。
