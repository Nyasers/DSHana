...
- **会话清单与打开**：「会话」区块列本 App 提交过的 DSH 会话（`GET /dshana/sessions`，投影见 `src/lib/session-list.ts`：坐标取宿主任务记录的 `metadata.dsh.sessionId`，按最近活动降序、上限 20），每条一个「在新窗口打开」按钮走 `POST /dshana/sessions/open`。后端用 `ctx.windows.create`（能力 `app/windows.manage`）开一个原生窗口，entry = `/stream.html?sid=<DSH 会话>&tid=<taskId>&appSurfaceSession=<调用方的票>`——票取自请求头 `X-Hana-App-Surface-Session`（`@hana/plugin-sdk` 的 client 只认 URL 里的这个参数），窗口里那份卡页才用得上同一套 client 连受管服务。这是**不依赖宿主入口**的打开路径：聊天流底部的任务 chip 点开去哪由宿主决定（宿主侧诉求单独附，不进仓）。
