...
- **会话卡（`ui/stream.html`）的生命周期**：卡面把 DSH 会话 id 写进查询串（`?sid=`），注入的 DSH UI 钉在那一段上，壳页在顶部挂一行跟踪态（取 `GET /dshana/card-state`：只在 `tracked` / `cancelling` 期间慢轮询 4s，其余状态停手）。**终态之后不断流**：这段会话多半是用户主动打开来翻看的历史会话，断掉或拒绝重开就等于看不了。主卡与 FP 不带 `sid`，不挂跟踪态。
- **闸门（中继侧，当前没有消费方）**：`src/runtime/bridge.ts` 在 WS 升级时取 `dshanaSid` / `dshanaTask` 票面并按票问 `gate(ticket)`（runtime 侧读宿主任务记录，终态即失活：失活即拒建、活流每 `GATE_RECHECK_MS` 复查）。这套判据当初是给流内卡补的（两端各一道：卡页自觉冻结盖当下会话，桥闸门盖「页面没自觉 / 事后重载」的面），现在**没有任何页面声明票面**（轻卡不读票面、窗口 entry 不带 `tid`），它不触发；保留是待清理，不是待复用。
...
- **会话清单与打开**：「会话」区块列本 App 提交过的 DSH 会话（`GET /dshana/sessions`，投影见 `src/lib/session-list.ts`：坐标取宿主任务记录的 `metadata.dsh.sessionId`，按最近活动降序、上限 20），每条一个「在新窗口打开」按钮走 `POST /dshana/sessions/open`。后端用 `ctx.windows.create`（能力 `app/windows.manage`）开一个原生窗口，entry = `/stream.html?sid=<DSH 会话>&appSurfaceSession=<调用方的票>`——票取自请求头 `X-Hana-App-Surface-Session`（`@hana/plugin-sdk` 的 client 只认 URL 里的这个参数），窗口里那份卡页才用得上同一套 client 连受管服务。**不带 `tid`**：卡页拿它当流票面，而中继闸门判的是「宿主任务还活跃」（终态即失活、拒建流），带上就等于打开历史会话时看不到东西；窗口走无票的流（与主卡 / FP 同侧，不闸）。这是**不依赖宿主入口**的打开路径：聊天流底部的任务 chip 点开去哪由宿主决定（宿主侧诉求单独附，不进仓）。
...
- **失败面**：`managed-runtime.ts` 的 `START_ERROR_HINTS` 按 code（port-busy / port-unreachable / boot-failed / deps / seed / not-authorized）给中文指引，壳页 action 态展示。
...
- **决策 J（活动模型 requestId 注册表 = globalThis，跨 bundle 共享）**：provider adapter
  ...
  流收尾注销；task-bridge 取消时按会话定向 hana.models.cancel（lib/model-requests.ts 只读
...
- src/lib/cancel-chain.ts：cancel 编排（planCancel/executeCancel/awaitCancelTerminal/
...
- src/runtime/main.ts：挂 approval-bridge；task-bridge 传 serviceBaseUrl；shutdown 停两桥。
...
- 单测新增 21 例（watch-sse 9 / task-map-ext 4 / cancel-chain 3 / approval-bridge 5）+
  ...
  node src/build.ts 与 node src-cordis/build.ts 通过（dist 内含 approval-bridge/requestApproval/
  cancel 链标记）。
...
- **ctx.routes.register**：单 bundle 只能 register 一次；registrar 收到 Hono sub-app（SKILL/
  ...
  runtime 单例——路由模块直接调 lib/managed-runtime.ts 是本设计前提）。
...
- src/lib/legacy-migrate.ts（纯 node 内置）+ scripts/migrate/legacy.mts CLI：
...
- 已测：node src/build.ts 与 node src-cordis/build.ts 通过（dist 含 ui/ + manifest cards +
